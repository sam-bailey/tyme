...
class GroupedTimeSeries:
    ...
    def _group_cols_to_id(self, **group_cols: Mapping[str, Any]) -> str:
        for col in self._group_columns:
            assert (
                col in group_cols.keys()
            ), f"{col} must be included in group columns. You passed {group_cols}"

        return "__".join(
            [
                "{}={}".format(col, str(group_cols[col]))
                for col in self._group_columns
            ]
        )
    ...
    def _group_id_to_cols(self, group_id: str) -> Mapping[str, Any]:
        ...
        for col in self._group_columns:
            assert (
                col in output.keys()
            ), f"{col} is missing from the group_id: {group_id}"

        return output

    def _process_timeseries_df(
        self, time_series_pd: pd.DataFrame
    ) -> pd.DataFrame:
        time_series_pd[self._time_column] = pd.to_datetime(
            time_series_pd[self._time_column]
        )
        time_series_pd = time_series_pd.sort_values(self._time_column)
        time_series_pd[self._group_id_column] = time_series_pd[
            self._group_columns
        ].apply(lambda x: self._group_cols_to_id(**x), axis=1)
        return time_series_pd

    def _calculate_frequency(self) -> np.timedelta64:
        possible_frequencies = (
            self._raw_pdf.groupby(self._group_columns)[self._time_column]
            .diff(periods=1)
            .value_counts()
        )
    ...
    def regression(
        ...
        for group_id, pdf_i in self._raw_pdf.groupby(self._group_id_column):
            ...
            X_lst += [ts_full[i : (i + lookback_window)] for i in range(max_i)]
            y_lst += [
                agg_func(
                    ts_full[
                        (i + lookback_window + min_predict_window - 1) : (
