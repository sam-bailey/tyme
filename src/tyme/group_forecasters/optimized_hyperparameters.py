...
from ..utils import GroupedTimeSeries
...
class GroupRegressor:
    ...
    def __init__(
        self,
        lookback_window: int,
        min_predict_window: int,
        max_predict_window: int,
        agg_func: Callable[[NumpyArray], float],
        loss_function: LossFunctions = LossFunctions(
            primary_loss="mae", monitoring_losses=["smape"]
        ),
        verbose: bool = False,
    ):

        assert (
            self._base_forecaster is not None
        ), "GroupRegressor must be subclassed and the _base_forecaster set"
    ...
    def _evaluate(
        ...
        y: NumpyArray,
        with_cov: bool = False,
        time_id: Optional[NumpyArray] = None,
        group_id: Optional[NumpyArray] = None,
    ) -> Union[
        Tuple[float, Mapping[str, float], NumpyArray],
        ...
        monitoring_losses = self._loss.monitoring_losses(
            y_pred=y_pred, y_true=y
        )

        if with_cov:
            df1 = pd.DataFrame(
                {"group_id": group_id, "time_id": time_id, "err": y_pred - y}
            )
            df2 = pd.DataFrame(
                {"group_id": group_id, "time_id": time_id, "err": y_pred - y}
            )
    ...
    def fit(
        ...
        starting_params: Optional[List[float]] = None,
    ) -> None:
        ...
        if starting_params is None:
            starting_params = (
                self._base_forecaster.get_default_starting_params()
            )
        ...
        def _minimize_me(params_lst, info):
            ...
            if (info["Nfeval"] % 10 == 0) & self._verbose:
                print(
                    "Params={0}  Loss={1:9f}   Monitoring Losses={2}".format(
        ...
        self._fitted_base_forecaster = self._base_forecaster.create_from_lst(
            self._fit_results.x
        )
    ...
    def forecast(self, timeseries: GroupedTimeSeries) -> pd.DataFrame:
        ...
        out_pdf = timeseries._raw_pdf[
            [
                timeseries._time_column,
                timeseries._target_column,
                timeseries._group_id_column,
            ]
        ]
        ...
        for group_id, pdf_i in timeseries._raw_pdf.groupby(
            timeseries._group_id_column
        ):
            time_max = pdf_i[timeseries._time_column].values[-1]

            y = pdf_i[timeseries._target_column].values[
                -self._lookback_window :
            ]
            self._fitted_base_forecaster.filter(y)
            forecast_y = self._fitted_base_forecaster.forecast(
                1, self._max_predict_window
            )
            forecast_time = [
                time_max + timeseries._frequency * (n + 1.0)
                for n in range(self._max_predict_window)
            ]
    ...
    def predict(self, timeseries: GroupedTimeSeries) -> pd.DataFrame:
        ...
        output_pdf = (
            timeseries._raw_pdf.sort_values(timeseries._time_column)
            .groupby(timeseries._group_id_column)
            .agg({timeseries._target_column: _filter_forecast_aggregate})
            .reset_index()
